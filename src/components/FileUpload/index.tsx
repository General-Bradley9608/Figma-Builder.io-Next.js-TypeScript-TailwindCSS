...
const FileUpload: React.FC<FileUploadProps> = ({ onFileUpload, uploadedFile }) => {
  ...
  return (
    <div
      {...getRootProps()}
      className="flex flex-col justify-center cursor-pointer mt-12 max-w-full text-black dark:text-primary-foreground rounded-lg bg-gray-300 bg-opacity-20 min-h-[290px] w-[594px] md:px-5 md:mt-10"
    >
      <input {...getInputProps()} />
      <div className="flex flex-col w-full md:max-w-full">
        ...
        <div className="flex flex-col mt-3 w-full md:max-w-full">
          {uploadedFile ? (
            ...
            <>
              <div className="flex flex-wrap gap-1 justify-center items-start w-full text-sm leading-none md:max-w-full">
