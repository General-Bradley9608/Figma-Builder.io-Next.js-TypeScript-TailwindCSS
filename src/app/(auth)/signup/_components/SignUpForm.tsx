  ...
  return (
    <section className="flex flex-col self-center w-full max-w-[416px]">
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-3xl font-semibold leading-none text-accent-foreground">
          Create your Play.CV account
        </h1>
        <p className="mt-2 text-sm leading-none text-secondary-foreground">
      ...
      <div className="flex gap-2 items-center p-1.5 mt-6 w-full text-base font-semibold bg-gray-50 rounded-lg border border-gray-100 border-solid">
        <button onClick={() => setAccountType('individual')} className={`overflow-hidden flex-1 shrink gap-2 self-stretch px-3.5 py-2.5 my-auto rounded-lg ${accountType === 'individual' ? 'text-white bg-indigo-600 shadow' : 'text-accent-foreground'}`}>
          I'm an Individual
        </button>
        <button onClick={() => setAccountType('entity')} className={`overflow-hidden flex-1 shrink gap-2 self-stretch px-3.5 py-2.5 my-auto rounded-lg ${accountType === 'entity' ? 'text-white bg-indigo-600 shadow' : 'text-accent-foreground'}`}>
          I'm an Entity
        </button>
      </div>
        ...
        {accountType === 'individual' ? (
          individualFormFields.map((field, index) => (
            <div key={index} className="flex flex-col mt-5 w-full">
              <label htmlFor={`field-${index}`} className="text-sm font-medium leading-none text-accent-foreground">
                {field.label}
              </label>
              ...
              {field.hint && <p className="mt-1.5 text-sm leading-none text-secondary-foreground">{field.hint}</p>}
            </div>
          ))) : (
